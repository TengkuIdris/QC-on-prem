...
function ProductParetoPage() {
  const navigate = useNavigate();
  const { setBreadcrumbItems } = useContext(BreadcrumbContext);
  const data = productDataMap[SearchParamsSelect.PARENTO];
  const pageUrl = "https://kznhub.com/product/pareto";
  ...
  useEffect(() => {
    setBreadcrumbItems([
      { label: "TOP", href: "/", onClick: () => navigate("/") },
      {
        label: data.title,
        href: "#",
        onClick: () => navigate(App.PRODUCT_PARETO),
      },
    ]);
...
export default ProductParetoPage;
