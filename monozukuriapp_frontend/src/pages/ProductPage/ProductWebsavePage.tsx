...
function ProductWebsavePage() {
  const navigate = useNavigate();
  const { setBreadcrumbItems } = useContext(BreadcrumbContext);
  const data = productDataMap[SearchParamsSelect.WEB_SAVE];
  const pageUrl = "https://kznhub.com/product/websave";
  ...
  useEffect(() => {
    setBreadcrumbItems([
      { label: "TOP", href: "/", onClick: () => navigate("/") },
      {
        label: data.title,
        href: "#",
        onClick: () => navigate(App.PRODUCT_WEBSAVE),
      },
    ]);
...
export default ProductWebsavePage;
