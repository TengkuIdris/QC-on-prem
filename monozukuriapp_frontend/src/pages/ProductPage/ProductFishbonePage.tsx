...
function ProductFishbonePage() {
  const navigate = useNavigate();
  const { setBreadcrumbItems } = useContext(BreadcrumbContext);
  const data = productDataMap[SearchParamsSelect.FISH_BONE];
  const pageUrl = "https://kznhub.com/product/fishbone";
  ...
  useEffect(() => {
    setBreadcrumbItems([
      { label: "TOP", href: "/", onClick: () => navigate("/") },
      {
        label: data.title,
        href: "#",
        onClick: () => navigate(App.PRODUCT_FISHBONE),
      },
    ]);
...
export default ProductFishbonePage;
