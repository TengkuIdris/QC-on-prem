...
function ProductServiceSupportPage() {
  const navigate = useNavigate();
  const { setBreadcrumbItems } = useContext(BreadcrumbContext);
  const data = productDataMap[SearchParamsSelect.SERVICE_SUPPORT];
  const pageUrl = "https://kznhub.com/product/service-support";
  ...
  useEffect(() => {
    setBreadcrumbItems([
      { label: "TOP", href: "/", onClick: () => navigate("/") },
      {
        label: data.title,
        href: "#",
        onClick: () => navigate(App.PRODUCT_SERVICE_SUPPORT),
      },
    ]);
...
export default ProductServiceSupportPage;
