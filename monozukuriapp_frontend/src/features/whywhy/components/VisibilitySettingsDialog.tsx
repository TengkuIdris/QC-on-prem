...
const VisibilitySettingsDialog: React.FC<VisibilitySettingsDialogProps> = ({
  ...
  const [publicScope, setPublicScope] = useState<"private" | "group" | "company">(
    (currentPublicScope as "private" | "group" | "company") || "company"
  );
  const [hideAuthorName, setHideAuthorName] = useState(currentHideAuthorName || false);
  const [isSaving, setIsSaving] = useState(false);

  useEffect(() => {
    if (open) {
      setPublicScope((currentPublicScope as "private" | "group" | "company") || "company");
      setHideAuthorName(currentHideAuthorName || false);
    }
  }, [open, currentPublicScope, currentHideAuthorName]);
